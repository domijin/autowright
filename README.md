...
- **Real scheduling** — cron expressions (with per-trigger IANA timezones and DST handled), one-shot
  "run once at…" triggers, run-on-app-start, and manual "Execute now" from the app or menu bar.
...
- **Menu-bar surface** — glance at what's running and what's next, and fire jobs without opening
  the window.
- **CLI** — full parity with the app: author automations as files (`automation pull`/`push`/
  `create`), execute and follow them, manage executions, secrets, triggers, and settings —
  headless-friendly and agent-friendly. Secret grants are explicit per automation
  (`--grant-secret`/`--grant-agent` flags); nothing is granted silently.
...
- **CLI** (`autowright`) — headless client of the same API, full UI parity; `autowright service`
  (launchd install/uninstall/status/restart) is what the app itself invokes at launch.
