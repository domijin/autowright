...
// the shared `ExecutionView` (../executionView) — plus skip-live-step,
// Cancel / Retry / Execute again. A §6 `queued` record renders the waiting
// state instead of that body.
...
import type { Execution, TriggerPayload } from '../types'
    ...
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <>
          ...
          {/* Execution card (§7): the shared STEPS rail + LOGS pane; the page
              adds the parameters block and the workspace link under the steps. */}
          <ExecutionView
            key={`${e.id}:${retryKey}`}
            executionId={e.id}
            full={full}
