...
# from scratch. Stops anything running from THIS REPO first (backend service,
# Electron, vite — deleting .venv under the live launchd KeepAlive service
# would otherwise break), then deletes the build artifacts: .venv,
# app/node_modules, app/dist, and build/ contents (keeping build/cache/ — the
# pinned CPython tarball is expensive to re-download — unless --cache). A
# backend service registered by the installed /Applications app is the user's
# live backend and is left untouched, as are the data dir
# (~/Library/Application Support/Autowright or AUTOWRIGHT_HOME) and the logs
# dir.
#
#   ./scripts/build-clean.sh            delete build artifacts, keep build/cache/
#   ./scripts/build-clean.sh --cache    also drop build/cache/ (removes build/ entirely)
set -euo pipefail
...
PLIST="$HOME/Library/LaunchAgents/com.autowright.backend.plist"
if [ -f "$PLIST" ] \
