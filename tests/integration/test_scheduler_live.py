...
def test_time_trigger_fires_and_is_consumed(backend, client):
    """§4.3/§6: a `time` trigger fires once via the tick loop, produces a
    'Once' execution, and vanishes from the trigger list. The harness runs the
    real loop at `AUTOWRIGHT_TICK_S=1` (§15), so the first tick baselines at
    ~1s and `at` sits just past it."""
    a = create_auto(client, name="Punctual")
    at = (datetime.now() + timedelta(seconds=3)).isoformat(timespec="seconds")
